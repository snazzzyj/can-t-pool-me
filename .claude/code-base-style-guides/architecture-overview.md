...
> **ID:** `PRINCIPLE` > **Context:** Foundational values that inform all architectural decisions

### Rules

| ID              | Principle                    | Implication                                      |
| --------------- | ---------------------------- | ------------------------------------------------ |
| `PRINCIPLE-001` | Favour immutability          | Never mutate; return new references              |
| `PRINCIPLE-002` | Prefer pure functions        | Same input → same output, no side effects        |
| `PRINCIPLE-003` | Composition over inheritance | Combine small functions; avoid class hierarchies |
| `PRINCIPLE-004` | Declarative over imperative  | Describe _what_, not _how_                       |
| `PRINCIPLE-005` | Explicit public APIs         | Export only what consumers need                  |
| `PRINCIPLE-006` | Co-location                  | Keep related code together                       |
| `PRINCIPLE-007` | Type safety at boundaries    | Validate external data with runtime checks       |

### Modern Pattern Mappings

```javascript
┌─────────────────────────────┬─────────────────────────────────────┐
│ Avoid                       │ Prefer                              │
├─────────────────────────────┼─────────────────────────────────────┤
│ Classes for services        │ Factory functions + closures        │
│ Abstract base classes       │ TypeScript interfaces + composition │
│ Class-based DI containers   │ React Context + custom hooks        │
│ Class methods on entities   │ Pure functions operating on types   │
│ Inheritance hierarchies     │ Composition + higher-order funcs    │
│ Mutable state in services   │ Immutable data + React Query/Zustand│
└─────────────────────────────┴─────────────────────────────────────┘
```
...
### Example: Domain API Layer

```typescript
// domains/chat/api/chat.api.ts
import { apiClient } from '@/shared/api/client';
import type { MessageDto, ThreadDto } from '../shared/types';

export const chatApi = {
  getThreads: () =>
    apiClient.get<ThreadDto[]>('/chat/threads'),

  getMessages: (threadId: string) =>
    apiClient.get<MessageDto[]>(`/chat/threads/${threadId}/messages`),

  sendMessage: (threadId: string, content: string) =>
    apiClient.post<MessageDto>(`/chat/threads/${threadId}/messages`, { content }),
} as const;
```

```typescript
// domains/chat/api/chat.queries.ts
...
```typescript
// domains/chat/models/message.model.ts
import type { MessageDto } from '../shared/types';

// Domain entity type (immutable)
...
// Factory function: DTO → Domain Entity
...
export function isFromUser(message: Message, userId: string): boolean {
  return message.authorId === userId;
}

export function isRecent(message: Message, thresholdMs = 60_000): boolean {
  return Date.now() - message.createdAt.getTime() < thresholdMs;
}

// Namespaced helpers (optional grouping)
...
```typescript
// domains/chat/shared/types/chat-state.types.ts
...
| ID         | Rule                                                                      |
| ---------- | ------------------------------------------------------------------------- |
| `TEST-001` | Co-locate tests with implementation                                       |
| `TEST-002` | Use factory functions for test data (not static objects)                  |
| `TEST-003` | Separate unit (`.test.ts`) and integration (`.integration.test.ts`) tests |
| `TEST-004` | Use MSW for API mocking                                                   |
| `TEST-005` | Custom render includes all necessary providers                            |

### Example: Test Data Factory

```typescript
// domains/chat/features/ChatBar/__fixtures__/chat-bar.fixtures.ts
...
- [ ] `CONFIG` — Add Zod-validated environment configuration
- [ ] `API` — Implement TanStack Query with domain query structure
- [ ] `FEATURE` — Standardise feature structure with explicit public APIs

### Phase 2: Refinement (Medium Priority)

- [ ] `DOMAIN` — Introduce functional domain models
- [ ] `LIB` — Add adapters for third-party dependencies
- [ ] `SHARED` — Apply 3+ domain rule; audit existing shared code

### Phase 3: Polish (Lower Priority)

- [ ] `DI` — Implement Context-based dependency injection
- [ ] `ERROR` — Add per-domain error boundaries
- [ ] `TEST` — Build comprehensive test infrastructure with factories
